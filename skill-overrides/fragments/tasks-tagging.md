   ...
   - **Phase-1 branch-setup tasks**: For each affected repo, generate a Phase-1 setup task with `[P]` (parallelizable) immediately at the top of Phase 1, *before* any other setup task. Format:
     ```text
     - [ ] T00X [P] [repo:<id>] Create feature branch via /speckit-multirepo-branch repo=<id> name=<BRANCH_NAME>
     ```
     ...
     - The configured `branch_prefix` for that repo is applied automatically by `/speckit-multirepo-branch`; do not bake it into the task text.
