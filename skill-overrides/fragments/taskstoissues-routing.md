...
.specify/extensions/multi-repo/scripts/bash/parse-repos-yaml.sh \
    | yq -r ".repos[] | select(.id == \"<id>\") | .github"
```

- If `github` is non-empty, create the issue against that `owner/repo` slug.
- If `github` is empty or `null`, fall back to the Spec Kit root repo remote and prefix the issue title with `[<repo_id>]` so reviewers know which repo it targets.
